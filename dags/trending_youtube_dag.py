...
default_args = {"owner": "windaru", "retries": 3, "retry_delay": timedelta(minutes=5)}


@dag(
    dag_id="trending_youtube_dag_v1",
...
def trending_youtube_dag():
    ...
    def fetch_trending_videos(
        region_code: str, max_results: int, target_file_path: str
    ):
    ...
    def data_processing(source_file_path: str, target_file_path: str):
        ...
        for video in videos_list:
            ...
            video["tags"] = (
                ", ".join(video["tags"]) if video["tags"] is not None else None
            )
            ...
            video["commentCount"] = (
                ...
                if video["commentCount"] is not None
                else None
            )
    ...
    fetch_trending_videos_task = fetch_trending_videos(
        region_code="ID", max_results=200, target_file_path=file_path
    )
    processed_file_path = "/opt/airflow/dags/tmp_file_processed.json"
    data_processing_task = data_processing(
        source_file_path=file_path, target_file_path=processed_file_path
    )
    load_to_bigquery_task = load_to_bigquery(
        source_file_path=processed_file_path, table_name="trending_videos"
    )
